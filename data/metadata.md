...
| `id` | YouTube video ID (11-char string; join key to `summaries/source_<id>_<slug>.md` in this wiki — build intermediates `transcripts/<id>_<slug>.md` and `metadata/<id>.json` not shipped in wiki). |
...
**Join key:** `id` → `summaries/source_<id>_<slug>.md` in this wiki (`resource: https://www.youtube.com/watch?v=<id>`); build intermediate `transcripts/<id>_<slug>.md` not present in wiki.
