...
`drake` is the predecessor to `targets` and the subject of the incubator's first pipeline meetup, [source__Cl4odbiG78](../../summaries/source__Cl4odbiG78_using-drake-to-develop-smart-reproducible-workflows.md), presented alongside the workflow principles in [source_TEUbVjhooIg](../../summaries/source_TEUbVjhooIg_project-oriented-workflow.md). The concepts transfer directly to `targets` (drake's successor): declare a plan of targets, let the package track dependencies, cache results, and rebuild only what changed, so a team can trust that `make()` produced the claimed outputs.
...
- **Plan as code**: `drake::drake_plan(data = read_data(), summary = summarise(data))` declares the DAG; `make(plan)` executes in order; `readd(data)` (now `tar_read()` in targets) retrieves — [source__Cl4odbiG78](../../summaries/source__Cl4odbiG78_using-drake-to-develop-smart-reproducible-workflows.md).
- **Caching and dependency tracking**: drake stores objects in `.drake/` (targets uses `_targets/`), hashes commands and dependencies, and skips up-to-date targets — [source__Cl4odbiG78](../../summaries/source__Cl4odbiG78_using-drake-to-develop-smart-reproducible-workflows.md).
- **Visualization**: `vis_drake_graph()` / `drake_config()` previews the build order, analogous to `tar_glimpse()` / `tar_network()` — [source__Cl4odbiG78](../../summaries/source__Cl4odbiG78_using-drake-to-develop-smart-reproducible-workflows.md).
