...
Code coverage is defined as the analysis of which lines are exercised by the test suite, used to guide safe refactoring of already-written code. The single source [source_bKRFCOkmpiw](../../summaries/source_bKRFCOkmpiw_testing-code-coverage.md) (continuing after the TDD session) argues: before touching a function such as `load_file()` you must ensure its behavior is covered by tests, otherwise refactoring can silently break it. The supporting package is `covr` (caption "cover package" → `covr`).
...
- **Definition**: ratio of executed lines/branches to total; by-file and by-package — [source_bKRFCOkmpiw](../../summaries/source_bKRFCOkmpiw_testing-code-coverage.md).
- **covr package** (`covr` ← "cover package"): `covr::package_coverage()`, `covr::file_coverage("R/load_file.R")`, `covr::report()`, `covr::percent_covered()` — [source_bKRFCOkmpiw](../../summaries/source_bKRFCOkmpiw_testing-code-coverage.md).
- **Report reading**: HTML `report()` shows green (covered) / red (uncovered) per line; console `percent_covered()` summarizes — [source_bKRFCOkmpiw](../../summaries/source_bKRFCOkmpiw_testing-code-coverage.md).
- **Refactor gate**: require coverage before restructuring `load_file()`-like functions — [source_bKRFCOkmpiw](../../summaries/source_bKRFCOkmpiw_testing-code-coverage.md).
