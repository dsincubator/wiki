...
Testing is introduced as the shift from manual inspection in the console to automated tests that can be run any number of times, crucial once a program exceeds what fits in one head and the impact of a change cannot be predicted. The series frames the work as "turning every mouse click into code" — painful at first but saving time from the mid-run onward. The single source for this topic ([source_9JcJFber21w](../../summaries/source_9JcJFber21w_testing-introduction.md), 59 views) anchors on Hadley Wickham's *Mastering Shiny* testing chapter (and *R Packages* Testing chapter) as the most up-to-date synthesis.
...
- **Manual vs automated**: console inspection is ephemeral; `testthat` tests are repeatable and fast — [source_9JcJFber21w](../../summaries/source_9JcJFber21w_testing-introduction.md).
- **Test anatomy**: intent string + toy data + `expect_*()` comparison — example `as_vector()` stripping names — [source_9JcJFber21w](../../summaries/source_9JcJFber21w_testing-introduction.md).
- **Four levels**: 1) functions (unit) 2) reactive 3) browser 4) visuals — focus on 1 and 4 — [source_9JcJFber21w](../../summaries/source_9JcJFber21w_testing-introduction.md).
- **Lowest-level principle**: test at the lowest feasible level for speed/stability; inject parameters to make design testable — [source_9JcJFber21w](../../summaries/source_9JcJFber21w_testing-introduction.md).
